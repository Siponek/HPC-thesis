...
The \exportSizeGzip~MB gzip-compressed total is competitive with typical Unity WebGL builds (50--100~MB uncompressed) despite being served uncompressed from the development server. Production deployment behind a reverse proxy (nginx, Cloudflare, GitHub Pages) with automatic gzip/brotli compression reduces the effective transfer to \exportSizeGzip~MB. This eliminates the need for Unreal Engine's Pixel Streaming infrastructure entirely, enabling deployment as a static web page with no GPU server requirements.
