...
\begin{itemize}
	...
	\item \textbf{OpenMP Simulation}: Multiple students work collaboratively at a shared desk with all cards visible in a common container, using private buffer areas for local sorting—demonstrating shared memory parallelism with thread-local storage
\end{itemize}

These physical activities were highly effective at conveying parallel computing concepts in a tangible, memorable way. However, they faced significant limitations: scalability (limited by desk size).
...
\begin{figure}[htbp]
	...
	\caption{Transformation from physical classroom experiments to digital serious game, preserving pedagogical effectiveness while overcoming physical limitations}
...
\begin{enumerate}
	...
	\item \textbf{Create Multiplayer Capability}: Implement real-time multiplayer functionality enabling 2--4 players to collaborate, simulating distributed computing scenarios

	\item \textbf{Evaluate Technical Feasibility}: Assess challenges and solutions for web-based multiplayer game development, particularly regarding state synchronization using the GDSync framework and HTTP/SSE relay architecture
...
\paragraph*{Multiplayer Mode (OpenMP Shared-Memory Parallelism):} Represents shared-memory parallelism where multiple threads cooperate on a single address space. All players work collaboratively with cards visible in a shared container (representing shared memory), each player uses private buffer zones for local sorting (representing thread-local storage), and players work simultaneously without forced turn-taking (representing OpenMP's implicit synchronization and parallel execution model).
...
\begin{figure}[htbp]
	\centering
	\includegraphics[width=0.9\textwidth]{figures/diagrams/sequential-vs-openmp-modes.png}
	\caption{Comparison of game modes: Single-player represents sequential execution (left) where one player sorts all cards independently; Multiplayer represents OpenMP shared-memory parallelism (right) where multiple players collaborate with a shared container and private buffers}
	\label{fig:game-modes-comparison}
\end{figure}
...
\begin{itemize}
	...
	\item \textbf{HTTP/SSE Relay Server}: Custom server for real-time web-based multiplayer communication
\end{itemize}
...
\begin{enumerate}
	...
	\item \textbf{Technical Implementation}: A functional web-based serious game demonstrating real-time multiplayer synchronization for OpenMP simulation, responsive UI/UX for displaying 50+ interactive elements across diverse screen sizes, and solutions to web platform constraints including state consistency and network latency management.

	\item \textbf{Comprehensive Documentation}: Detailed analysis of technical challenges (particularly GDSync framework issues, multiplayer synchronization, and web browser constraints) and solutions, providing practical guidance for educational game developers.
...
\textbf{Chapter~\ref{ch:background}} reviews parallel computing fundamentals (focusing on OpenMP shared-memory parallelism), serious games in education, web game development challenges, and multiplayer architecture patterns, identifying gaps in existing HPC educational tools.
...
\textbf{Chapter~\ref{ch:architecture}} presents the system design and architecture, including scene structure, component design, multiplayer networking, and responsive UI/UX considerations.

\textbf{Chapter~\ref{ch:implementation}} details the technical implementation, including development environment, single-player and multiplayer implementation, and web-specific optimizations.

\textbf{Chapter~\ref{ch:problems}} provides an honest analysis of difficulties encountered during development, including GDSync framework issues, multiplayer synchronization challenges, and web browser constraints.
