...
In this thesis, the artifact is intentionally presented as a \textbf{game}. At the same time, for pedagogical precision, it can be categorized as a \textbf{serious game} and used as an instructor-guided educational tool in classroom or workshop settings.
...
These physical activities were highly effective at conveying parallel computing concepts in a tangible, memorable way. However, they faced significant limitations, including scalability (limited by classroom size).
...
The tool is built using open-source technologies:

\begin{itemize}
	\item \textbf{Godot Engine 4.x}: Lightweight engine with strong 2D and web export support
	\item \textbf{GDScript}: Python-like scripting language for rapid iteration
	\item \textbf{GDSync Framework}: Synchronization framework used for multiplayer state handling
	\item \textbf{HTTP/SSE Relay Server}: Browser-compatible real-time relay for multiplayer communication
\end{itemize}
