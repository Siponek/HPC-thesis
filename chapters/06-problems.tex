...
\begin{itemize}
    ...
    \item Viewport culling to avoid rendering off-screen cards (partially implemented via Godot's built-in culling)
    \item Texture atlases, level-of-detail (LOD), and adjustable quality settings were considered but deferred as profiling showed rendering bottlenecks were manageable for the target card counts (see Chapter~\ref{ch:results}, Section~\ref{subsec:framerate})
\end{itemize}
