...
\begin{itemize}
    ...
    \item Provide public buffers for each player (thread-local storage)
...
\paragraph*{Multiplayer Mode (OpenMP Shared-Memory Parallelism):} Represents shared-memory parallelism where multiple threads cooperate on a single address space. All players work collaboratively with cards visible in a shared container (representing shared memory), each player uses public buffer zones for local sorting (representing thread-local storage), and players work simultaneously without forced turn-taking (representing OpenMP's implicit synchronization and parallel execution model).
...
\begin{figure}[htbp]
    ...
    \caption{Game Mode~2: OpenMP Shared-Memory Parallelism. Multiple players (threads) access a shared card container concurrently and use public buffers for thread-local sorting.}
    \label{fig:openmp-mode}

    \vspace{0.3em}
    \begin{description}
        \item[Pedagogical mapping:] \textbf{Players} represent concurrent threads working on a single task. The \textbf{shared container} models a globally accessible memory space, introducing concepts such as race conditions and the need for synchronization (barriers). \textbf{public buffers} correspond to variables declared as \texttt{private} or \texttt{threadprivate}, allowing lock-free local sorting before merging back to shared space.
    \end{description}
\end{figure}
...
\begin{table}[htbp]
    ...
    \begin{tabular}{@{}p{3.5cm}p{4cm}p{5cm}@{}}
        ...
        \midrule
        Thread-Local Storage   & Student's private notebook         & Player's public buffer zones         \\
        \midrule
