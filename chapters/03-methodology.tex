...
\begin{enumerate}
    ...
    \item \textbf{Smallest Web Exports}: The final game build is approximately 28~MB uncompressed (12~MB gzip-compressed), loading in 3--5~seconds on typical broadband. This is 2--4$\times$ smaller than Unity WebGL builds and orders of magnitude smaller than Unreal's Pixel Streaming infrastructure requirement.
...
\begin{itemize}
    ...
    \item \textbf{Custom logging framework}: A structured logging system that timestamps and categorizes network events, gameplay actions, and synchronization operations into separate log channels. This enabled post-session analysis of timing issues (e.g., determining whether a card move arrived before or after a barrier signal) that could not be diagnosed from real-time observation alone.
