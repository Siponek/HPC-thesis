...
\begin{itemize}
    ...
    \item \textbf{const\_generator (Godot plugin)}: Automatically generates GDScript constant files from project resources (scene paths, node names, input actions). This eliminated manual string duplication---when a scene path changed, the generated constants updated automatically, preventing hard-to-debug runtime errors from stale path strings \citep{const_generator_asset}.
