...
This chapter describes the detailed technical implementation of the HPC Sorting Serious Game, covering both single-player and multiplayer modes, key algorithms, code structure, and mobile-specific optimizations. Selection rationale for tools and frameworks is discussed in Chapter~\ref{ch:methodology}, especially Section~\ref{sec:technology-selection}.
...
\end{lstlisting}
External tool and framework links used in implementation are listed in Appendix~\ref{app:d:tool-links}.
...
\label{subsec:impl-card-component}
...
\label{subsec:impl-card-manager}
...
Buffers represent thread-local storage in OpenMP shared-memory parallelism.
...
\end{enumerate}
% TODO: Add sequence diagram for lobby lifecycle (create/join room, signaling, host handshake, ready state, game start).

%-----------------------------------------------------------------------
\subsection{GDSync Integration}
\label{subsec:impl-gdsync-integration}

GDSync provides high-level networking abstractions. The decision process for adopting GDSync over alternatives is documented in Section~\ref{subsec:gdsync-selection}; this section focuses on implementation details. For this thesis implementation, the project uses an author-maintained GDSync fork that includes a fix required for the web-export workflow \citep{gdsync_fork}. A pull request with this fix was submitted upstream \citep{gdsync_framework}; until upstream integration is complete, the fork remains the operational dependency.
...
In multiplayer mode (OpenMP simulation), players should not see cards in other players' private buffers, representing thread-private data.
...
% TODO: Add figure or listing with real logger output from a multiplayer session (INFO/WARN/ERROR with timestamps and context).
% Logger source: local implementation in this repository (add external URL only if replaced by third-party plugin)
...
VarTree plugin allows real-time inspection and modification of variables during gameplay for debugging purposes. This tool displays GDSync connection state variables such as client ID, host status, current lobby, and player count.
VarTree plugin reference: \citep{vartree_asset}.
