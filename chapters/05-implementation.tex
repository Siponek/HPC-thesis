...
The complete source code is available in the project repository \citep{github_project}.
...
Card generation supports configurable counts (1--200) with optional unique or repeated values. The initialization sequence in \texttt{\_ready()} follows a deterministic pipeline:
...
func _ready():
    ...
    sorted_all = values.duplicate() # Create a separate sorted array for reference
    sorted_all.sort()
...
\begin{lstlisting}[caption={Multiplayer initialization with host/client (multiplayer\_card\_manager.gd)}]
func _ready():
...
\texttt{CardState} captures whether a card is in the shared container or in a player's local buffer, and who owns it. This explicit serialization boundary ensures that network payloads are minimal and self-describing, and that deserialization on the receiving client produces a deterministic scene state.
...
\end{lstlisting}
% TODO add screenshot of logger output with multiple components
...
\begin{table}[htbp]
    ...
    \caption{Key source file locations (\texttt{MP/} = \texttt{scenes/Multiplayer/})}
    ...
    \begin{tabular}{@{}p{4.2cm}p{7.5cm}@{}}
        ...
        Multiplayer Card Manager  & \texttt{MP/MultiplayerGame/multiplayer\_card\_manager.gd} \\
        Barrier Manager           & \texttt{MP/MultiplayerGame/barrier\_manager.gd}           \\
        Connection Manager        & \texttt{MP/connection\_manager.gd}                        \\
        Multiplayer Lobby         & \texttt{MP/Lobby/multiplayer\_lobby.gd}                   \\
        GDSync Web Patch          & \texttt{MP/GDSyncWebPatch/gdsync\_web\_patch.gd}          \\
        Signaling LocalServer     & \texttt{MP/GDSyncWebPatch/local\_server\_signaling.gd}    \\
        Signaling Client          & \texttt{MP/GDSyncWebPatch/signaling\_client.gd}           \\
