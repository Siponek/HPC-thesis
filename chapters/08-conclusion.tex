...
% \begin{figure}[htbp]
%     \centering
%     \includegraphics[width=0.9\textwidth]{figures/diagrams/contributions-summary-overview}
%     \caption{Overview of the three contribution areas and their interrelationships.}
%     \label{fig:contributions-summary}
% \end{figure}
...
\begin{itemize}
    \item Functional web-first serious-game-based educational tool demonstrating real-time multiplayer synchronization
    ...
    \item Integration of GDSync framework with an HTTP/SSE relay networking approach for browser-compatible multiplayer
    \item UI/UX design patterns for displaying and manipulating numerous interactive elements across varying viewport sizes and input methods
\end{itemize}
...
\paragraph*{4. Qualitative Validation}
...
\begin{itemize}
    ...
    \item Methodical tool and framework selection, documented in Chapter~\ref{ch:methodology}, improved implementation reliability and reduced rework risk
\end{itemize}
...
\begin{itemize}
    ...
    \item local buffers representing thread-local storage (OpenMP)
    \item Card movement between shared container and local buffers representing memory access patterns
...
\textbf{Q: How can we implement real-time multiplayer interaction in a web-first educational tool with acceptable performance?}

\textbf{A:} The thesis demonstrated that:
\begin{itemize}
    \item Browser-compatible HTTP/SSE relay networking enabled robust multiplayer synchronization in web deployment
    ...
    \item Target 60 FPS was achieved for typical scenarios in modern desktop-class browsers
\end{itemize}
...
\textbf{Q: How can we balance educational effectiveness with engagement and playability in a web-first environment?}

\textbf{A:} The thesis indicates that:
\begin{itemize}
    ...
    \item Informal testing suggests good engagement while supporting introductory educational goals
\end{itemize}
...
\paragraph*{Browser/Platform Constraints Are Real:}

\begin{itemize}
    \item Design for varying viewports and input methods (mouse, touch) from the start
    \item Performance optimization is essential, especially for WebAssembly targets
    \item Browser security policies (mixed content blocking, Private Network Access) require careful deployment configuration
    \item Test on diverse browsers and hardware, not just development machines
\end{itemize}
...
Numerous opportunities exist for extending and improving this work. Figure~\ref{fig:future-work-roadmap} presents a phased roadmap with dependency links between items.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.95\textwidth]{figures/diagrams/future-work-roadmap}
    \caption{Future work roadmap organized by time horizon, with dependencies between items.}
...
\begin{enumerate}
    \item \textbf{Guided Algorithm Modes}: While the tool already supports any parallel sorting strategy through its algorithm-agnostic design (Chapter~\ref{ch:architecture}, Section~\ref{sec:algorithm-mode-extensibility}), future work could add \emph{guided} modes that walk students through specific algorithms step-by-step \citep{sanders2019sequential}:
          \begin{itemize}
              \item \textbf{Guided Parallel Merge Sort}: System-enforced phase progression (divide $\to$ local sort $\to$ merge rounds) with automatic partition assignment and barrier triggers, building on the strategy players can already execute freely.
              \item \textbf{Guided Parallel Quicksort}: System-provided pivot selection UI, partition validation, and automatic sub-group assignment, adding structure to the quicksort strategy players can already perform manually.
              \item Sample sort (distributed-memory style, future MPI extension)
              \item Bitonic sort
          \end{itemize}

          Table~\ref{tab:future-algo-mode-comparison} summarizes the pedagogical distinctions between the two primary strategies and what guided modes would add.

          \begin{table}[htbp]
              \centering
              \caption{Parallel sorting strategies supported by the tool and potential guided-mode enhancements}
              \label{tab:future-algo-mode-comparison}
              \begin{tabular}{@{}p{3.5cm}p{5cm}p{5cm}@{}}
                  \toprule
                  \textbf{Aspect}   & \textbf{Merge Sort}                             & \textbf{Quicksort}                                     \\
                  \midrule
                  Already supported & Buffers for local sort, barriers for merge sync & Shared container for partition, sub-group coordination \\
                  Guided mode adds  & Auto partition assignment, phase enforcement    & Pivot UI, partition validation, auto sub-group split   \\
...
\paragraph*{Web Distribution Enhancements:}
...
\begin{itemize}
    ...
    \item Integration with study materials, classroom activities, and assessment rubrics provided in Appendix~\ref{app:d:classroom-activities}
\end{itemize}
...
\begin{enumerate}
    \item Design and implementation of a web-based serious game for HPC education
...
\begin{itemize}
    \item Scope centered on web-first browser deployment; native app packaging was not a primary implementation target
...
\begin{enumerate}
    \item \textbf{Serious-game-based tools can support HPC concept learning} through intuitive mechanics that make abstract ideas concrete and manipulable.

    \item \textbf{Web-first platforms are viable for educational multiplayer tools}, despite technical challenges in synchronization, UI/UX design, and performance optimization.
    ...
    \item \textbf{Interdisciplinary challenges inspire innovative solutions} at the intersection of computer science education, game design, parallel computing, and web deployment.
\end{enumerate}
...
\begin{center}
    \textit{``People overestimate what they can do in a year and underestimate what they can do in ten years.''}\\
    \textit{— Bill Gates}
\end{center}
