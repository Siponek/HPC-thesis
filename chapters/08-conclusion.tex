...
\begin{enumerate}
    \item \textbf{Guided Algorithm Modes}: The tool currently supports static partitioning strategies such as parallel merge sort through its existing mechanics (Chapter~\ref{ch:architecture}, Section~\ref{sec:algorithm-mode-extensibility}). Future work could add \emph{guided} modes that walk students through specific algorithms step-by-step \citep{sanders2019sequential}:
          \begin{itemize}
              \item \textbf{Guided Parallel Merge Sort}: System-enforced phase progression (divide $\to$ local sort $\to$ merge rounds) with automatic partition assignment and barrier triggers, adding structure to the strategy players can already execute freely.
              \item \textbf{Guided Parallel Quicksort}: This would require \emph{new game mechanics} not present in the current tool: a pivot selection UI, partition validation, recursive sub-round management, and sub-group barrier synchronization. This represents a significant architectural extension rather than a guided overlay on existing mechanics.
              \item Sample sort (distributed-memory style, future MPI extension)
              \item Bitonic sort
          \end{itemize}

          Table~\ref{tab:future-algo-mode-comparison} summarizes the pedagogical distinctions and implementation effort.

          \begin{table}[htbp]
              \centering
              \caption{Potential guided-mode enhancements for parallel sorting strategies}
              \label{tab:future-algo-mode-comparison}
              \begin{tabular}{@{}p{3.5cm}p{5cm}p{5cm}@{}}
                  \toprule
                  \textbf{Aspect}  & \textbf{Merge Sort}                          & \textbf{Quicksort}                                                       \\
                  \midrule
                  Current support  & Playable with existing mechanics             & \textbf{Not supported}---requires new mechanics                          \\
