...
\begin{lstlisting}[language={}, caption={Architectural mechanism: private buffer visibility gate (pseudocode)}]
...
\begin{description}
    ...
    \item[Private player buffers (thread-local storage).] Each player has private buffer slots for temporarily holding cards. The buffer contiguity validation enforces that a player works on a logically coherent subset of the data. Players naturally use buffers for local sorting (merge sort) or for staging cards during partitioning (quicksort).
...
\begin{enumerate}
    ...
    \item \textbf{Local sort}: Each player moves their assigned cards into their private buffer, sorts them locally, then returns them to the shared container in sorted order. All players do this simultaneously---embarrassingly parallel work with no inter-player communication.
...
\begin{figure}[htbp]
    ...
    \caption{How players perform parallel merge sort using the existing tool: divide cards among players, sort locally in private buffers, then merge sorted runs through barrier-gated rounds}
...
\begin{itemize}
    ...
    \item \emph{Embarrassingly parallel local sort}: independent work in private buffers $\equiv$ no inter-thread communication
...
\begin{table}[htbp]
    ...
    \begin{tabular}{@{}p{3.8cm}p{5.2cm}p{5.2cm}@{}}
        ...
        Buffer usage       & Heavy---local sorting in private buffers & Light---partition done in shared container \\
