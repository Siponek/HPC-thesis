...
\begin{description}
    ...
    \item[Public player buffers (thread-local storage).] Each player has public buffer slots for temporarily holding cards. The buffer contiguity validation enforces that a player works on a logically coherent subset of the data. Players naturally use buffers for local sorting (merge sort) or for staging cards during partitioning (quicksort).
