...
\textbf{CodeCombat} teaches programming through RPG-style gameplay in which players write real Python or JavaScript code to control characters; it is used in thousands of classrooms worldwide.
\textbf{Lightbot} introduces sequencing, loops, and procedures through a visual puzzle interface that requires no programming syntax, making it accessible to young learners.

\paragraph*{Parallel Computing Games:}

Few games specifically target parallel computing education. Notable exceptions include PARMACS Simulator (interactive visualization of parallel algorithms), ParSim (an educational tool for parallel algorithm design), and CUDA Learning Games (GPU programming challenges). However, these tools typically focus on visualization rather than hands-on, game-like interaction, and few provide multiplayer experiences.
