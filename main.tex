...
% --- Memory (measured via Chrome Task Manager: ~530 MB baseline across all states) ---
% No per-state variables needed — memory is engine-dominated, reported as prose in 06-results.tex

% (Load time and event latency reported as prose in 06-results.tex, no variables needed)
...
High Performance Computing (HPC) and parallel programming pose significant pedagogical challenges, requiring students to internalize abstractions at the beginning without tangible interactive experiences. Serious games facilitate intuitive understanding through engagement, surfacing misconceptions and enabling targeted instruction.

This thesis presents an educational serious game with the goal to support teachers in presenting fundamental parallel computing concepts—specifically OpenMP (shared memory)—through interactive card-sorting mechanics in web browsers. The game materializes abstract concepts through concrete manipulation: players sort cards collaboratively in multiplayer mode with local buffer zones, simulating OpenMP shared memory with thread-local storage. Students cooperate via a Web interface to minimize sorting time under constraints, learning concepts like speedup, scalability, communication overhead and multithreading.

The development process involved the identification of a suitable cross-platform game engine, the design of extensible networking mechanisms, and iterative prototyping to balance educational goals with technical feasibility.

Key contributions include: (1) original mapping between card-sorting mechanics and HPC concepts derived from physical classroom experiments, (2) robust multiplayer state synchronization for educational purposes using GDSync, (3) responsive UI/UX solutions for manipulating 50+ interactive elements across screen sizes in web browsers, (4) comprehensive documentation of technical challenges and solutions regarding GDSync and multiplayer synchronization, and (5) an open-source, extensible platform for HPC education research suitable for conference publication and scientific journals.

\textbf{Keywords:} Serious Games, High Performance Computing, Parallel Computing Education, OpenMP,
%MPI,
Web-Based Game Development, Multiplayer Synchronization, Godot Engine, GDScript, Card Sorting, Educational Technology
