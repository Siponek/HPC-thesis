...
\begin{itemize}
    \item \textbf{Browser}: Chromium-based browser (Chrome, Edge, Brave) or Firefox, version 90 or later
    \item \textbf{WebGL}: WebGL 2.0 support required
    \item \textbf{SharedArrayBuffer}: Must be enabled (requires HTTPS with proper COOP/COEP headers)
    ...
    \item \textbf{Input}: Mouse or touchscreen; keyboard shortcuts optional
\end{itemize}

\subsection{Desktop Version (Windows)}

\begin{itemize}
    \item \textbf{Operating System}: Windows 10 or later (x86\_64)
    \item \textbf{RAM}: 2~GB minimum
    \item \textbf{Storage}: 100~MB free space
    \item \textbf{Graphics}: Vulkan or OpenGL 3.3 support
    \item \textbf{Network}: Internet connection for multiplayer
\end{itemize}
...
\noindent\textbf{Note:} The web export requires the hosting server to set specific HTTP headers (\texttt{Cross-Origin-Opener-Policy: same-origin} and \texttt{Cross-Origin-Embedder-Policy: require-corp}) for \texttt{SharedArrayBuffer} support. If these headers are missing, multiplayer functionality will not work. See docs/GITHUB\_PAGES\_AND\_LOCAL\_SIGNALING\_LIMITATIONS.md in the project repository for details.

\subsection{Desktop Version}

\begin{enumerate}
    \item Download the Windows executable from the project releases page
    \item Extract the archive if necessary
    \item Run the executable directly---no installation is required
\end{enumerate}
...
\begin{itemize}
    ...
    \item \textbf{Settings}: Configure theme and display options
\end{itemize}
...
\begin{enumerate}
    ...
    \item Configure game parameters in the options dialog:
          \begin{itemize}
              ...
              \item \textbf{Buffer slot count}: number of private buffer positions
...
\paragraph{Private Buffers (Thread-Local Storage):}
\begin{itemize}
    \item Each player has private buffer slots visible only to themselves
...
\begin{itemize}
    ...
    \item Ensure the hosting server sends correct COOP/COEP headers for \texttt{SharedArrayBuffer}
...
\begin{itemize}
    ...
    \item The desktop version generally performs better than the web version for large card counts
\end{itemize}
...
\begin{itemize}
    ...
    \item Facilitate post-gameplay discussion connecting game mechanics to parallel programming:
          \begin{itemize}
              ...
              \item Private buffers $\leftrightarrow$ thread-local / private variables
