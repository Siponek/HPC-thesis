...
\begin{enumerate}
    ...
    \item Each thread can have public (local) data in buffers
...
\begin{enumerate}
    ...
    \item What role does the shared container play compared to public buffers?
...
\begin{table}[htbp]
    ...
    \begin{tabular}{@{}p{4cm}p{5cm}p{5cm}@{}}
        \toprule
        \textbf{Game Element}         & \textbf{HPC/OpenMP Concept} & \textbf{Code Example}          \\
        \midrule
        Single-player mode            & Sequential execution        & Single-threaded program        \\
        Multiplayer session           & Parallel region             & \texttt{\#pragma omp parallel} \\
        Main card container           & Shared array                & \texttt{shared(array)}         \\
        public buffer zones           & Thread-public storage       & \texttt{public(buffer)}        \\
        Players working independently & Independent threads         & No explicit synchronization    \\
        Sorting within buffer         & Local computation           & Serial code in parallel region \\
        Merging back to main          & Critical section            & \texttt{\#pragma omp critical} \\
        Barrier mechanic              & Barrier synchronization     & \texttt{\#pragma omp barrier}  \\
        Main thread during barrier    & Master thread               & \texttt{\#pragma omp master}   \\
        \bottomrule
    \end{tabular}
\end{table}
